...
public class OrderServiceTest {
    ...
    public void 주문취소() throws Exception {
        //given
        Member member = createMember();
        Book item = createBook("studyJPA", 10000, 10);
        ...
        assertEquals("주문이 취소된 상품은 재고가 다시 채워져야한다.", 10, item.getStockQuantity());
    }
    ...
    public void 상품주문_재고수량초과() throws Exception {
        //given
        Member member = createMember();
        Item item = createBook("JPA study", 10000, 10);
    ...
    private Member createMember() {
        ...
        member.setAddress(new Address("Seoul", "hanam", "123-123"));
        em.persist(member);
        return member;
    }
}

// 좋은 테스트는.. db 등 없이 메서드로 순순한 단위테스트를 진행하는것이 좋다.
// 지금은 jpa가 동작하는 방식을 볼 수 있는 테스트
